...
TEST(RngCudaNormal, CpuCudaParityContiguous) {
#if VBT_WITH_CUDA
    ...
    for (float mean : means) {
      for (float std : stds) {
        for (std::int64_t N : sizes) {
          ...
          vbt::rng::cpu::normal_(t_cpu, mean, std, vbt::rng::default_cpu());
          vbt::rng::cuda::normal_(t_cuda, mean, std, vbt::rng::default_cuda(dev));
          // Copy CPU tensor to host
          std::vector<float> host_cpu;
          if (N > 0) {
            host_cpu.resize(static_cast<std::size_t>(N));
            std::memcpy(host_cpu.data(), t_cpu.data(), static_cast<std::size_t>(N) * sizeof(float));
          }
          ...
          for (std::size_t i = 0; i < host_cpu.size(); ++i) {
            EXPECT_NEAR(host_cpu[i], host_cuda[i], 1e-5f);
          }
        }
      }
    }
