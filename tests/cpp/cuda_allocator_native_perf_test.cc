...
void print_help(const char* prog) {
  std::cout << "Usage: " << prog
            << " --scenario B1|B2|B3 --device <idx> [options]\n"
            << "Options:\n"
...
ScenarioId parse_scenario_id(const std::string& s) {
  if (s == "B1") return ScenarioId::B1;
  if (s == "B2") return ScenarioId::B2;
  if (s == "B3") return ScenarioId::B3;
...
int run_main_impl(int argc, char** argv) {
  ...
  // Native binary only supports B1-B3.
  if (sid == ScenarioId::B4) {
    std::cerr << "B4 is not supported by native perf binary (use async binary)\n";
    return 2;
  }
  ...
  if (backend_kind == "async") {
    std::cerr << "async allocator backend configured; native perf binary requires native backend\n";
    return 77;
  }
  if (backend_kind == "none") {
    std::cerr << "No CUDA backend available; skipping perf run\n";
    return 77;
  }
  ...
  cfg.counts = counts;
  cfg.num_replays = (mode == RunMode::Smoke || mode == RunMode::Normal || mode == RunMode::Heavy)
                        ? args.num_replays
                        : 1;
  ...
  try {
    switch (sid) {
      ...
      case ScenarioId::B4:
        ...
        throw std::runtime_error("B4 not supported by native binary");
    }
  } catch (const std::exception& ex) {
