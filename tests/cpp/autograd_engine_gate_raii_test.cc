...
TEST(AutogradGateRaiiTest, BackwardThrowsThenNextBackwardRuns) {
  ...
  // Second backward: must still complete (gate must be released).
  {
    TensorImpl leaf = make_cpu_dense_f32({4}, 0.0f);
    auto* meta = vbt::autograd::get_autograd_meta(leaf, /*create_if_missing=*/true);
    ASSERT_NE(meta, nullptr);
    ...
    auto Id = vbt::core::make_intrusive<FunctionNode>(
        "Id",
        meta1,
        [](std::vector<OptionalTensor>&& gin) {
          std::vector<OptionalTensor> out(1);
    ...
    ensure_next_edges_sized(*Id);
    Id->next_edges[0] = vbt::autograd::Edge{intrusive_ptr<vbt::autograd::Node>(acc.get()), 0};
    ...
    ensure_next_edges_sized(*Root);
    Root->next_edges[0] = vbt::autograd::Edge{intrusive_ptr<vbt::autograd::Node>(Id.get()), 0};
