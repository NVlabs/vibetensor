# SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0

import os
import pathlib
import numpy as np
import pytest

import vibetensor.torch as vt


def _find_so(name: str) -> str | None:
    so_name = f"lib{name}.so"
    candidates = [
        pathlib.Path.cwd() / so_name,
        pathlib.Path(__file__).resolve().parent.parent.parent / so_name,
        pathlib.Path(__file__).resolve().parent.parent.parent / "build-py" / so_name,
    ]
    env = os.environ.get(f"VBT_{name.upper()}_PATH")
    if env:
        return env
    for p in candidates:
        if p.exists():
            return str(p)
    return None


def test_ok_null_raises_runtime_error():
    path = _find_so("ok_null")
    if path is None:
        pytest.skip("ok_null plugin not found; ensure build produced libok_null.so")
    vt.ops.load_library(path)

    a = np.arange(4, dtype=np.float32)
    ta = vt.from_dlpack(a)
    with pytest.raises(RuntimeError) as ei:
        _ = vt.ops.ext.bad(ta)
    assert "returned OK but out==NULL" in str(ei.value)
