...
// Progress marker used by C++ tests to observe plugin init stage boundaries.
// 0 = not started
// 1 = entered init
// 2 = staged def + kernel, sleeping
// 3 = returning OK
_Atomic int vbt_p3_stage_sleep_state = 0;

int vbt_p3_stage_sleep_get_state(void) {
  return atomic_load(&vbt_p3_stage_sleep_state);
}
...
static vt_status vt_p3_stage_sleep_cpu(vt_stream /*s*/,
                                      vt_tensor a,
                                      vt_tensor b,
                                      vt_tensor* out) {
  if (!out) {
...
vt_status vbt_plugin_init(const struct vbt_host_api* host,
  ...
  out_api->name = "p3_stage_sleep";

  atomic_store(&vbt_p3_stage_sleep_state, 1);

  if (host->register_library) host->register_library("vt");
  if (host->def) host->def("vt::p3_stage_sleep(Tensor, Tensor) -> Tensor");

  vt_status st = VT_STATUS_INTERNAL;
  if (host->register_cpu_kernel2) {
    st = host->register_cpu_kernel2("vt::p3_stage_sleep", &vt_p3_stage_sleep_cpu);
  }
  if (st != VT_STATUS_OK) return st;

  atomic_store(&vbt_p3_stage_sleep_state, 2);

#if defined(__unix__) || defined(__APPLE__)
  // Sleep long enough for the host test to observe mid-init state.
  usleep(200 * 1000);
#endif

  atomic_store(&vbt_p3_stage_sleep_state, 3);
  return VT_STATUS_OK;
}
