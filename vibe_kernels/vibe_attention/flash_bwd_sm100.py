...
from vibe_attention import barrier, copy_utils, pipeline, utils
from vibe_attention.blackwell_helpers import gemm_ptx_w_idx, gemm_w_idx  # noqa
from vibe_attention.block_info import BlockInfo
from vibe_attention.mask import AttentionMask
from vibe_attention.named_barrier import NamedBarrierBwdSm100
...
class FlashAttentionBackwardSm100:
    ...
    def kernel(
        ...
        sQt = cute.make_tensor(
            cute.recast_ptr(sQ.iterator, sQt_layout.inner), sQt_layout.outer
        )
        ...
        sdOt = cute.make_tensor(
            cute.recast_ptr(sdO.iterator, sdOt_layout.inner), sdOt_layout.outer
        )
    ...
    def dQacc_reduce(
        ...
        assert (
            cute.size(tdQrdQ_t2r_shape, mode=[1]) == self.dQaccum_reduce_stage
        ...
        while work_tile.is_valid_tile:
            ...
            for m_block in cutlass.range(m_block_min, m_block_max, unroll=1):
                ...
                if const_expr(self.deterministic):
                    barrier.wait_eq(
                        mdQ_semaphore_cur[m_block, None].iterator, tidx, 0, n_block
                    )
