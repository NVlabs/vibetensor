...
from vibe_attention import copy_utils, hopper_helpers as sm90_utils, pipeline, utils
from vibe_attention.block_info import BlockInfo
from vibe_attention.hopper_helpers import gemm_w_idx, gemm_zero_init
from vibe_attention.mask import AttentionMask
from vibe_attention.named_barrier import NamedBarrierBwd, NamedBarrierFwd
from vibe_attention.seqlen_info import SeqlenInfoQK
from vibe_attention.tile_scheduler import (
    ParamsBase,
    SingleTileScheduler,
...
class FlashAttentionBackwardSm90:
    ...
    def __call__(
        ...
        mLSE, mdPsum, mdQaccum = [
            utils.select(t, LSE_dPsum_dQaccum_transpose)
    ...
    def kernel(
        ...
        if warp_idx < 4:
            ...
            if warp_idx == 1:
                ...
                self.dQaccum_store(
                    mdQaccum, sdQaccum, block_info, TileSchedulerCls, SeqlenInfoCls
                )
        else:
    ...
    def load(
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                load_V(
                    tma_bar_ptr=pipeline_dO.producer_get_barrier(producer_state_dO_cur)
                )
    ...
    def epilogue_dKV(
        ...
        sdV = (
            sV if const_expr(not self.dKV_swapAB) else utils.transpose_view(sV)
        )  # reuse sV SMEM
        ...
        sdK = (
            sK if const_expr(not self.dKV_swapAB) else utils.transpose_view(sK)
        )  # reuse sK SMEM
