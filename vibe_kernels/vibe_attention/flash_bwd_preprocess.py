...
from vibe_attention import copy_utils, utils
        ...
        if work_tile.is_valid_tile:
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                ...
                mO_cur = cute.domain_offset(
                    (seqlen.offset_q, 0), mO[None, num_head, None]
                )
                mdO_cur = cute.domain_offset(
                    (seqlen.offset_q, 0), mdO[None, num_head, None]
                )

                padded_offset_q = seqlen.offset_q + batch_size * self.m_block_size
                mdPsum_cur = cute.domain_offset(
                    (padded_offset_q,), mdPsum[num_head, None]
                )
                headdim_v = mO.shape[2]
            ...
            if cutlass.const_expr(mLSE is not None):
                if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    mLSE_cur = cute.domain_offset(
                        (seqlen.offset_q,), mLSE[num_head, None]
                    )
            ...
            if cutlass.const_expr(mLSE is not None):
                if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    mLSElog2_cur = cute.domain_offset(
                        (padded_offset_q,), mLSElog2[num_head, None]
                    )
