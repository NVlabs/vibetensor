...
def dswiglu(
    ...
    if const_expr(not isinstance(x, tuple)):
        ...
        d_silu_x_dout = (
            sigmoid_x - silu_x * sigmoid_x
        ) * dout + silu_x_dout  # FFMA, FFMA
        ...
        d_silu_x_dout = utils.fma_packed_f32x2(
...
def dswiglu_oai(
    ...
    *,
    ...
    if const_expr(not isinstance(x, tuple)):
        ...
        # FFMA, FFMA, FMUL
        d_silu_x_dout = (
            sigmoid_alpha_x + alpha * (silu_x - silu_x * sigmoid_alpha_x)
        ) * dout
        ...
        tanh_alpha_x_half = (tanh(alpha_x_half[0]), tanh(alpha_x_half[1]))
        sigmoid_alpha_x = utils.fma_packed_f32x2(
            tanh_alpha_x_half, (0.5, 0.5), (0.5, 0.5)
        )
...
def dreglu(
    ...
    if const_expr(not isinstance(x, tuple)):
        ...
        dx = (
            (dout_y[0] if x0_pos else Float32(0.0)),
            (dout_y[1] if x1_pos else Float32(0.0)),
        )
