    ...
    def copy_fn(src_idx, dst_idx, pred: bool = False):
        ...
        for m in cutlass.range_constexpr(tAcA.shape[1]):
            ...
            if const_expr(is_even_m_smem) or tApA_m[m]:
                ...
                cute.copy(
                    thr_copy_A, mA_row[None, ki], tAsA[(None, m), dst_idx], pred=tApA_k
                )
...
def gather_k_get_copy_fn(
    ...
    def prefetch_from_gmem_fn(
        src_idx, pred: bool = False
            ...
            for m in cutlass.range_constexpr(tAcA.shape[1]):
                if tApA_m[m]:
                    cute.copy(
                        ...
                        pred=(
                            None
                            if const_expr(tApA_k_pred is None)
                            else tApA_k_pred[None, k]
                        ),
                    )

    return copy_fn, (
        prefetch_from_gmem_fn
        if const_expr(gAIdx is not None)
