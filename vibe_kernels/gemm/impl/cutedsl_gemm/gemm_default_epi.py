...
class GemmDefaultEpiMixin:
    ...
    def epi_begin(
        ...
        if const_expr(params.mRowVecBroadcast is not None):
            ...
            num_copy_elems = (
                const_expr(max(32, rowvec_dtype.width)) // rowvec_dtype.width
            )
        ...
        if const_expr(params.mColVecBroadcast is not None):
            ...
            num_copy_elems = (
                const_expr(max(32, colvec_dtype.width)) // colvec_dtype.width
            )
        ...
        if const_expr(
            params.mRowVecBroadcast is not None or params.mColVecBroadcast is not None
        ):
    ...
    def epi_begin_loop(
        ...
        if const_expr(tDsRowVec is not None):
            ...
            tDrRowVec = cute.make_fragment(
                tDsRowVec_cur.layout, tDsRowVec_cur.element_type
            )
            cute.autovec_copy(
                cute.filter_zeros(tDsRowVec_cur), cute.filter_zeros(tDrRowVec)
            )
        ...
        if const_expr(tDsColVec is not None):
            ...
            # tDrRowVec = cute.make_fragment_like(tDsRowVec_cur)
            tDrColVec = cute.make_fragment(
                tDsColVec_cur.layout, tDsColVec_cur.element_type
            )
            cute.autovec_copy(
                cute.filter_zeros(tDsColVec_cur), cute.filter_zeros(tDrColVec)
            )
    ...
    def epi_smem_bytes_per_stage(
        ...
        row_vec_smem_size = (
            0 if args.mRowVecBroadcast is None else cta_tile_shape_mnk[1]
        )
        col_vec_smem_size = (
            0 if args.mColVecBroadcast is None else cta_tile_shape_mnk[0]
        )
        row_vec_dtype = (
            args.mRowVecBroadcast.element_type
            if args.mRowVecBroadcast is not None
            else Float32
        )
        col_vec_dtype = (
            args.mColVecBroadcast.element_type
            if args.mColVecBroadcast is not None
            else Float32
        )
        return (
            row_vec_smem_size * row_vec_dtype.width
            + col_vec_smem_size * col_vec_dtype.width
        ) // 8

    def epi_get_smem_struct(self, params: EpilogueParams):
        row_vec_smem_size = (
            0 if params.mRowVecBroadcast is None else self.cta_tile_shape_mnk[1]
        )
        col_vec_smem_size = (
            0 if params.mColVecBroadcast is None else self.cta_tile_shape_mnk[0]
        )
        row_vec_dtype = (
            params.mRowVecBroadcast.element_type
            if params.mRowVecBroadcast is not None
            else Float32
        )
        col_vec_dtype = (
            params.mColVecBroadcast.element_type
            if params.mColVecBroadcast is not None
            else Float32
        )

        @cute.struct
        class EpiSharedStorage:
            sRowVec: cute.struct.Align[
                cute.struct.MemRange[row_vec_dtype, row_vec_smem_size], 16
            ]
            sColVec: cute.struct.Align[
                cute.struct.MemRange[col_vec_dtype, col_vec_smem_size], 16
            ]
    ...
    def epi_get_smem_tensors(
        ...
        sRowVec = None
        if const_expr(params.mRowVecBroadcast is not None):
            sRowVec = storage.epi.sRowVec.get_tensor(
                cute.make_layout(self.cta_tile_shape_mnk[1])
            )
        sColVec = None
        if const_expr(params.mColVecBroadcast is not None):
            sColVec = storage.epi.sColVec.get_tensor(
