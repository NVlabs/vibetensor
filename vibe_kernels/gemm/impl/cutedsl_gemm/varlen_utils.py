...
class VarlenManager:
    ...
    class Params(ParamsBase):
        ...
        @cute.jit
        def create(
            args: VarlenArguments, *, loc=None, ip=None
        ) -> "VarlenManager.Params":
    ...
    def to_underlying_arguments(args: VarlenArguments, *, loc=None, ip=None) -> Params:
        assert not (
            args.mCuSeqlensM is not None and args.mCuSeqlensK is not None
    ...
    def create(
        ...
        *,
        ...
        if const_expr(varlen_m or varlen_k):
            ...
            tensormap_workspace_idx = cute.make_layout(cute.arch.grid_dim())(
                cute.arch.block_idx()
            )
            if const_expr(varlen_m):
                ...
                if const_expr(has_D):
                    tensormap_d_ptr = tensormap_manager.get_tensormap_ptr(
                        params.tensormaps[
                            tensormap_workspace_idx, tensormap_d_idx, None
                        ].iterator
                    )
    ...
    def len_m(self, batch_idx: Int32) -> Int32:
        if const_expr(self.varlen_m):
            return (
                self.params.cu_seqlens_m[batch_idx + 1]
                - self.params.cu_seqlens_m[batch_idx]
            )
        else:
    ...
    def len_k(self, batch_idx: Int32) -> Int32:
        if const_expr(self.varlen_k):
            return (
                self.params.cu_seqlens_k[batch_idx + 1]
                - self.params.cu_seqlens_k[batch_idx]
            )
        else:
    ...
    def offset_batch_AIdx(self, batch_idx: Int32) -> cute.Tensor:
        params = self.params
        if const_expr(self.varlen_m):
            mAIdx_mk = cute.domain_offset(
                (params.cu_seqlens_m[batch_idx],), params.mAIdx
            )
        elif const_expr(self.varlen_k):
            mAIdx_mk = cute.domain_offset(
                (params.cu_seqlens_k[batch_idx],), params.mAIdx
            )
        else:
    ...
    def init_tensormap_epi(
        ...
        if const_expr(self.varlen_m):
            ...
            for tma_atom, tensormap_epi_ptr in zip(
                tma_atoms_epi, self._tensormap_epi_ptrs
            ):
                self.tensormap_manager.init_tensormap_from_atom(
        ...
        if const_expr(self.varlen_k):
            ...
            if self._is_group_changed:
                ...
                if const_expr(not self.gather_A):
                    ...
                    orders.insert(
                        0, 0 if const_expr(a_layout == LayoutEnum.ROW_MAJOR) else 1
                    )
    ...
    def update_tensormap_epi(
        ...
        if const_expr(self.varlen_m):
            ...
            order_d = (
                (0 if const_expr(d_layout.is_m_major_c()) else 1)
    ...
    @cute.jit
    def fence_tensormap_update_epi(
        self, is_manager_warp: bool | Boolean = True
    ) -> None:
        if const_expr(self.varlen_m):
            if self._is_group_changed and is_manager_warp:
                if const_expr(self._tensormap_d_ptr is not None):
