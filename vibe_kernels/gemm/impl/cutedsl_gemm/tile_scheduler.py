...
class TileScheduler:
    ...
    class Params(ParamsBase):
        ...
        ) -> "TileScheduler.Params":
            assert args.cluster_shape_mnk[2] == 1
            cluster_shape_mn = const_expr(
                cute.select(args.cluster_shape_mnk, mode=[0, 1])
            )
            problem_shape_ntile_mn = cute.select(
                args.problem_shape_ntile_mnl, mode=[0, 1]
            )
            problem_shape_ncluster_mn = cute.ceil_div(
                problem_shape_ntile_mn, cluster_shape_mn
            )
    ...
    def get_grid_shape(
        ...
        *,
        ...
        num_ctas_mnl = tuple(
            x * y
            for x, y in zip(params.problem_shape_ncluster_mnl, params.cluster_shape_mn)
    ...
    def _swizzle_cta(
        ...
        if group_id < params.num_groups_regular:
            ...
            cid_slow, cid_fast_in_group = params.group_size_tail_divmod.divmod(
                id_in_group
            )
    ...
    def get_current_work(self, *, loc=None, ip=None) -> cutlass.utils.WorkTileInfo:
        ...
        if const_expr(not params.is_persistent):
            ...
            is_valid = self._current_work_linear_idx < cute.size(
                ...
                if is_scheduler_warp:
                    ...
                    if lane_idx < cute.size(params.cluster_shape_mn):
                        ...
                        if const_expr(cute.size(params.cluster_shape_mn) == 1):
                            self._tile_count[self._pipeline_state.index] = (
                                current_work_linear_idx
                            )
                            self._scheduler_pipeline.producer_commit(
                            ...
                            utils.store_shared_remote(
                                ...
                                smem_ptr=self._tile_count.iterator
                    ...
                    current_work_linear_idx = self._tile_count[
                        self._pipeline_state.index
                    ]
    ...
    def producer_tail(self):
        if const_expr(
            self.params.is_persistent and self.params.tile_count_semaphore is not None
        ):
...
class TriangularTileScheduler(TileScheduler):
    ...
    class Params(ParamsBase):
        ...
        def create(
            ...
            return TriangularTileScheduler.Params(
                ...
                FastDivmod.create(
                    (group_size_tail if group_size_tail > 0 else 1) * group_size
                ),
                args.tile_count_semaphore if const_expr(args.is_persistent) else None,
                cluster_shape_mn,
                args.is_persistent,
    ...
    def get_grid_shape(
        ...
        *,
        ...
        num_ctas_mnl = tuple(
            x * y for x, y in zip(clusters, params.cluster_shape_mn)
        ) + (params.problem_shape_ncluster_mnl[2],)
        if const_expr(not params.is_persistent):
            return num_ctas_mnl
        else:
    ...
    def get_current_work(self, *, loc=None, ip=None) -> cutlass.utils.WorkTileInfo:
        ...
        group_id = (
            utils.ceil(
                (utils.sqrt(2 * cluster_id_in_problem + 2.25) - 0.5)
        ...
        if group_id < params.num_groups_regular:
            group_col, group_remainder = params.group_size_mul_group_size_divmod.divmod(
                id_in_group
            )
        else:  # tail part
            group_col, group_remainder = (
                params.group_size_tail_mul_group_size_divmod.divmod(id_in_group)
            )
        ...
        if id_in_group >= group_size_actual * group_size * group_id:  # triangular tail
            ...
            if group_id < params.num_groups_regular:
                cid_n_in_group, cid_m_in_group = params.group_size_divmod.divmod(
                    group_remainder
                )
            else:
...
class VarlenMTileScheduler(TileScheduler):
    ...
    def get_grid_shape(
        ...
        *,
        ...
        total_clusters_m_max = (
            params.total_m + num_batch * (block_size - 1)
        ) // block_size
        ...
        if const_expr(not params.is_persistent):
            ...
            num_persistent_clusters = cutlass.min(
                max_active_clusters, total_clusters_max
            )
            return (*params.cluster_shape_mn, num_persistent_clusters)
    ...
    def _swizzle_cta(
        ...
        cid_fast_in_group, cid_slow = Int32(0), Int32(0)
        if const_expr(
            params.group_size_divmod is not None
            and params.group_size_tail_divmod is not None
        ):
            num_clusters = num_clusters_m * params.problem_shape_ncluster_mnl[1]
            if (group_id + 1) * num_clusters_in_group <= num_clusters:
                cid_slow, cid_fast_in_group = params.group_size_divmod.divmod(
                    id_in_group
                )
            else:  # tail part
                cid_slow, cid_fast_in_group = params.group_size_tail_divmod.divmod(
    ...
    def get_current_work(self, *, loc=None, ip=None) -> cutlass.utils.WorkTileInfo:
        ...
        if batch_idx >= num_batch:
            ...
            num_clusters_m = cute.arch.shuffle_sync(
                num_clusters_m, batch_idx_in_problems
            )
            ...
            cid_m, cid_n = self._swizzle_cta(
                cluster_id_in_problem, num_clusters_m, loc=loc, ip=ip
            )
        ...
        if const_expr(self.params.tile_count_semaphore is not None):
            ...
            if is_scheduler_warp:
                if cute.arch.lane_idx() == 0:
                    ...
                    current_work_linear_idx = (
                        num_persistent_clusters
                        + utils.atomic_add_i32(1, params.tile_count_semaphore)
                    )
