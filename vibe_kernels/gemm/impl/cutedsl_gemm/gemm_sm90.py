...
from vibe_kernels.gemm.impl.cutedsl_gemm import (
    copy_utils,
...
class GemmSm90:
    ...
    def __call__(
        ...
        tile_sched_args = self.get_scheduler_arguments(
            mA, mB, mD, scheduler_args, varlen_args
        )
        ...
        epi_smem_size = (
            ...
            if self.is_persistent and mD is not None
            else 0
        )
        epi_c_smem_size = (
    ...
    def kernel(
        ...
        if const_expr(has_D):
            if const_expr(not self.is_persistent):
                sD_ptr = cute.recast_ptr(
                    sA.iterator, epi_smem_layout.inner, dtype=self.d_dtype
                )
                ...
                while work_tile.is_valid_tile:
                    ...
                    if const_expr(not self.gather_A):
                        ...
                        if const_expr(varlen_m):
                            ...
                            assert varlen_k
                            # (tile_K, RestK)
                            gAIdx = cute.flat_divide(
                                mAIdx_mk, (self.cta_tile_shape_mnk[2],)
                            )
                            # (tile_M, K)
                    ...
                    if const_expr(not self.gather_A):
                        ...
                        tidx = (
                            ...
                            - cute.arch.WARP_SIZE * self.ab_load_warp_id
                        )
        ...
        if warp_idx < self.ab_load_warp_id:
            ...
            k_tile_cnt_static = cute.ceil_div(
                mA_mkl.shape[1], self.cta_tile_shape_mnk[2]
            )
