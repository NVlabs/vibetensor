...
class GemmWrapperBase:
    ...
    def validate_tensor(tensor: Tensor, name: str, ndim: int) -> None:
        assert (
            tensor.dim() == ndim and tensor.is_cuda
        ), f"{name} must be a {ndim}D CUDA tensor"
        assert tensor.dtype in torch2cute_dtype_map, f"Unsupported dtype for {name}"

    @staticmethod
    def validate_shape(
        tensor: Tensor, expected_shape: Tuple[int, ...], name: str
    ) -> None:
        assert (
    ...
    def validate_and_prepare_tensors(
        ...
        assert not (
            cu_seqlens_m is not None and cu_seqlens_k is not None
        ...
        if gather_A:
            assert (
                cu_seqlens_m is not None or cu_seqlens_k is not None
        ...
        if cu_seqlens_m is not None:
            ...
            assert (
                total_K == K_B
            ), f"K dimension mismatch: expected {total_K}, B has {K_B}"
    ...
    def determine_major_orders(
        ...
        for name, dims in major_configs.items():
            if name in tensors and tensors[name].tensor is not None:
                tensors[name].major = GemmWrapperBase.get_major_order(
                    tensors[name].tensor, dims
                )
    ...
    def create_scheduler_args(
        ...
        return TileSchedulerOptions(
            ...
            batch_idx_permute=(
                (
                    from_dlpack(batch_idx_permute, assumed_align=4).mark_layout_dynamic(
                        leading_dim=0
                    )
                )
    ...
    def create_varlen_args(
        ...
        if num_tensormaps > 0:
            like = cu_seqlens_m if cu_seqlens_m is not None else cu_seqlens_k
            assert like is not None
            tensormaps = alloc.empty(
                (num_blocks, num_tensormaps, tensormap_size),
                dtype=torch.int64,
                like=like,
            )
            tensormaps_cute = from_dlpack(
                tensormaps, assumed_align=128
        ...
        return VarlenArguments(
            mCuSeqlensM=(
                from_dlpack(cu_seqlens_m, assumed_align=4).mark_layout_dynamic(
                    leading_dim=0
                )
                if cu_seqlens_m is not None
                else None
            ...
            mCuSeqlensK=(
                from_dlpack(cu_seqlens_k, assumed_align=4).mark_layout_dynamic(
                    leading_dim=0
                )
