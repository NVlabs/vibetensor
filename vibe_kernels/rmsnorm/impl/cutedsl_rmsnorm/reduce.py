...
def row_reduce(
    ...
    warp_op = {
        ...
        cute.ReductionOp.MAX: (
            cute.arch.fmax if cutlass.const_expr(x.dtype == Float32) else max
        ),
...
def online_softmax_reduce(
    ...
    mbar_ptr: Optional[cute.Pointer] = None,
    hook_fn: Optional[Callable] = None,
    phase: Optional[cute.Int32] = None,
    ...
    if cutlass.const_expr(reduction_buffer is not None):
        ...
        if cutlass.const_expr(warps_per_row > 1 or cluster_n > 1):
            ...
            if cutlass.const_expr(mbar_ptr is None):
                if lane_idx == 0:
                    reduction_buffer[row_idx, col_idx] = utils.f32x2_to_i64(
                        max_x, sum_exp_x
                    )
                cute.arch.barrier()
                ...
                for i in cutlass.range_constexpr(num_iter):
                    ...
                    if idx < cute.size(reduction_buffer, mode=[1]):
                        max_x_single_warp[i], sum_exp_x_single_warp[i] = (
                            utils.i64_to_f32x2(reduction_buffer[row_idx, idx])
                        )
