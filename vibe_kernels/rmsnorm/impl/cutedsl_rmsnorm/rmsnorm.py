...
@torch.library.custom_op(
    "kernel_factory::_cutedsl_rmsnorm_fwd",
...
def _rmsnorm_fwd(
    ...
    """RMSNorm forward pass using CuTeDSL backend."""
...
def rmsnorm_fwd(
    ...
    out = alloc.empty_like(x, dtype=out_dtype)
    rstd = (
        alloc.empty((x.shape[0],), like=x, dtype="float32")
        if store_rstd
        else None
    )
    if residual is not None:
    ...
    if residual is not None or (
        ...
        residual_out = alloc.empty_like(x, dtype=residual_dtype if residual_dtype is not None else x.dtype)
    else:
        residual_out = None
...
def rmsnorm_ref(x, w=None, bias=None, residual=None, eps=1e-6):
    ...
    if residual is not None:
        ...
        x_f32 += residual_f32
    x_norm = x_f32 / (
        torch.sqrt(torch.mean(x_f32.square(), dim=-1, keepdim=True) + eps)
    )
...
class RMSNormBackward(ReductionBase):
    ...
    def kernel(
        ...
        if row < M:
            ...
            tXgdO_cur = utils.coord_offset_i64(bidx_start, tXgdO, dim=3)[
                None, None, None, 0
            ]
        ...
        for bidx in cutlass.range(bidx_start, cute.ceil_div(M, tiler_mn[0]), gdim):
            ...
            if row + gdim * tiler_mn[0] < M:
                ...
                tXgdO_cur = utils.coord_offset_i64(bidx + gdim, tXgdO, dim=3)[
                    None, None, None, 0
                ]
                copy(
                    tXgX_cur,
            ...
            if row < M or tiler_mn[0] == 1:
                rstd = mRstd[row]
            if const_expr(mdResO is not None):
                tXgdResO_cur = utils.coord_offset_i64(bidx, tXgdResO, dim=3)[
                    None, None, None, 0
                ]
                if row < M or tiler_mn[0] == 1:
            ...
            if const_expr(mdRes is not None):
                ...
                tXgdRes_cur = utils.coord_offset_i64(bidx, tXgdRes, dim=3)[
                    None, None, None, 0
                ]
                if row < M or tiler_mn[0] == 1:
...
@torch.library.custom_op(
    "kernel_factory::_cutedsl_rmsnorm_bwd",
...
def _rmsnorm_bwd(
    ...
    """RMSNorm backward pass using CuTeDSL backend."""
    assert x.dim() == 2, "Input must be 2D"
    assert x.is_cuda, "Input tensor must be on CUDA device"
    assert x.dtype in [
        torch.float16,
...
def rmsnorm_bwd(
    ...
    if dresidual_out is not None and dresidual_out.dtype != dx.dtype:
        dresidual = alloc.empty_like(x, dtype=dresidual_out.dtype)
    else:
        dresidual = None
...
class RMSNormFunction(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout, *args):  # type: ignore[override]
...
def rmsnorm(
    ...
    """CuTeDSL RMSNorm with automatic differentiation support."""
    return RMSNormFunction.apply(
        x, weight, bias, residual, out_dtype, residual_dtype, eps, prenorm
    )


class CuTeDSLRMSNorm(torch.nn.RMSNorm):
    """RMSNorm module backed by the CuTeDSL implementation."""
