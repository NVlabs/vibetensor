...
def _torch_rotary(
    ...
    # cos/sin gathered to shape (B, H, S, D//2)
...
def main() -> None:
    ...
    print("Rotary QK Triton vs Torch (bf16)")
    print("================================")
    ...
    for name, B, H, S, D in CONFIGS:
        ...
        def triton_fn():
            return apply_rotary_embedding(q, k, cos, sin, positions, backend="triton")

        torch_ms = _time_fn(torch_fn)
        triton_ms = _time_fn(triton_fn)

        ref_q, ref_k = torch_fn()
        tri_q, tri_k = triton_fn()
        ...
        print(f"  triton_mean_ms  = {triton_ms:.4f}")
        print(f"  triton_speedup  = {torch_ms / triton_ms:.3f}x vs torch")
